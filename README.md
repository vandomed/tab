...
``` r
tabmulti(Age + Sex + Race ~ Group, data = tabdata, latex = TRUE) %>% kable(format = "html")
```
...
fit %>% tabglm(factor.compression = 5, latex = TRUE) %>% kable(format = "html")
```
...
fit %>% tabglm(factor.compression = 3, latex = TRUE) %>% kable(format = "html")
```
